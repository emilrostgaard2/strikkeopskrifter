  ...
  function render(){
    ...
    document.getElementById('shops').innerHTML = list.length ? list.map((s,i)=>`
      <div class="shop ${i===0?'best':''}">
        ${s.logo?`<img class="slogo" src="${s.logo}" alt="" width="40" height="40">`:`<span class="slogo logo-txt">${s.s[0]}</span>`}
  ...
  fetch(ROOT+'data/priser.json').then(r=>r.ok?r.json():null).then(P=>{
    ...
    if(box && P[box.dataset.garn]){
      ...
      box.innerHTML=Object.values(g.shops).filter(x=>x.price).map((x,i)=>{
        ...
        return `<div class="shop ${i===0?'best':''}">${x.logo?`<img class="slogo" src="${x.logo}" alt="" width="40" height="40">`:`<span class="slogo logo-txt">${x.shop[0]}</span>`}<div class="name">${x.shop}<small>${note}</small></div>
